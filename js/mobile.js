window.NILTASK_MOBILE_VERSION = 'v207';
...
async function _activity(p) {
    ...
    const timelineCard = (it) => {
        ...
        const btn = it.act ? (it.act.k==='task'
            ? `<button class="af-btn primary" ${openData(it)}>📂 Open Task</button>`
            : `<button class="af-btn primary" ${openData(it)}>🚀 Open Room</button>`) : '';
        return `<div class="af-card ${it.cls}${seen?' seen':''}" data-v207-icon="${it.emoji||'⚡'}" ${openData(it)}>
    ...
    const html = `<div class="mScr-inner ${mode==='attention'?'nf-mode':'af-mode'}">
      <div class="m-hdr m-hdr-plain" style="display:flex;align-items:center;justify-content:space-between;">
        <div class="m-htitle">${mode==='attention'?'🔔 Notifications':'🗞️ Activity'} <span class="af-version">v207</span> ${(mode==='attention'&&unread)?`<span style="background:#2563eb;color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:30px;margin-left:6px;vertical-align:middle;">${unread} new</span>`:''}</div>
...
// v207 presentation overrides
(function(){
  if(document.getElementById('mobile-activity-v207-css')) return;
  const st=document.createElement('style');st.id='mobile-activity-v207-css';
  st.textContent='\n/* ── NILTASK ACTIVITY / NOTIFICATIONS v207 — VERSION 1 ─────────────────── */\n.af-version{\n  display:inline-flex;align-items:center;margin-left:6px;padding:3px 6px;\n  border-radius:999px;background:rgba(99,102,241,.10);color:var(--accent,#6366f1);\n  font-size:8px;font-weight:950;vertical-align:middle;\n}\n.af-mode,.nf-mode{background:var(--bg-body,#f6f7fb);}\n.af-mode .m-hdr,.nf-mode .m-hdr{\n  min-height:58px;padding-left:14px;padding-right:14px;background:var(--bg-body,#fff);\n  border-bottom:1px solid var(--border,#e5e7eb);\n}\n.af-mode .m-htitle,.nf-mode .m-htitle{font-size:17px;font-weight:950;}\n.af-mode .af-filters,.nf-mode .af-filters{\n  position:sticky;top:58px;z-index:18;display:grid;\n  grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;overflow:visible;\n  margin:9px 10px 3px;padding:9px;border:1px solid var(--border,#dbe2ea);\n  border-radius:15px;background:var(--bg-body,#fff);\n  box-shadow:0 4px 14px rgba(15,23,42,.06);\n}\n.af-mode .af-select,.nf-mode .af-select{\n  width:100%;min-width:0;height:38px;box-sizing:border-box;\n  padding:0 22px 0 7px;border:1px solid var(--border,#cbd5e1);\n  border-radius:10px;color:var(--text,#111827);background:var(--surface,#f8fafc);\n  font-size:9.5px;font-weight:850;text-overflow:ellipsis;\n}\n.af-mode .af-feed,.nf-mode .af-feed{\n  padding:13px 11px 96px;gap:0;background:var(--bg-body,#f6f7fb);\n}\n.af-mode .af-div{margin:12px 0 9px;}\n.af-mode .af-div span,.nf-mode .af-div span{\n  color:var(--text-secondary,#64748b);background:transparent;padding:0;\n  font-size:9px;font-weight:950;text-transform:uppercase;letter-spacing:.08em;\n}\n.af-mode .af-card{\n  min-height:88px;margin-bottom:16px;padding:13px 38px 13px 60px;\n  border:1px solid var(--border,#e1e6ef);border-left-width:5px;border-radius:18px;\n  background:var(--bg-body,#fff);\n  box-shadow:0 2px 5px rgba(15,23,42,.04),0 10px 26px rgba(15,23,42,.07);\n}\n.af-mode .af-card::after{\n  content:attr(data-v207-icon);position:absolute;top:14px;left:13px;\n  width:35px;height:35px;display:grid;place-items:center;border-radius:50%;\n  color:#fff;background:#6366f1;font-size:15px;font-weight:950;\n  box-shadow:0 5px 12px rgba(15,23,42,.13);\n}\n.af-mode .af-card.orange::after{background:#f97316;}\n.af-mode .af-card.green::after{background:#16a34a;}\n.af-mode .af-card.purple::after{background:#7e22ce;}\n.af-mode .af-card.blue::after{background:#2563eb;}\n.af-mode .af-title{\n  padding-right:3px;font-size:13px;line-height:1.45;font-weight:850;\n}\n.af-mode .af-meta{margin-top:6px;font-size:9.5px;line-height:1.5;}\n.af-mode .af-actions{margin-top:9px;}\n.af-mode .af-btn{\n  min-height:35px;padding:7px 11px;border-radius:10px;font-size:10px;font-weight:900;\n}\n.nf-mode .nf-row{\n  margin:0 0 9px;padding:12px;border:1px solid var(--border,#e1e6ef);\n  border-radius:15px;background:var(--bg-body,#fff);\n  box-shadow:0 3px 12px rgba(15,23,42,.05);\n}\n.nf-mode .nf-row.unread{\n  border-left:5px solid var(--accent,#6366f1);\n  background:linear-gradient(90deg,rgba(99,102,241,.08),var(--bg-body,#fff) 60%);\n}\n.nf-mode .nf-ic{width:40px;height:40px;border-radius:13px;}\n.nf-mode .nf-title{font-size:12px;line-height:1.45;font-weight:850;}\n.nf-mode .nf-time{margin-top:5px;font-size:9px;}\nhtml[data-theme="dark"] .af-mode,html[data-theme="dark"] .nf-mode,\nhtml[data-theme="dark"] .af-mode .af-feed,html[data-theme="dark"] .nf-mode .af-feed{\n  background:#090a0d;\n}\nhtml[data-theme="dark"] .af-mode .af-card,html[data-theme="dark"] .nf-mode .nf-row{\n  background:#15161b;border-color:#292b33;box-shadow:none;\n}\n@media(max-width:380px){\n  .af-mode .af-filters,.nf-mode .af-filters{\n    margin-left:7px;margin-right:7px;padding:7px;gap:5px;\n  }\n  .af-mode .af-select,.nf-mode .af-select{font-size:8.5px;padding-left:5px;}\n}\n';
