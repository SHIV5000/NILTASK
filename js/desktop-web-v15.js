 ...
 * Desktop acceptance corrections:
 * - reminder RPC is the single reminder/reply owner (no duplicate client reply)
 * - task FILE replies show icon + filename, never the private storage path
 * - Chat / Tasks conversation switch lives in the fixed chat top bar
 * - Activity has Mark all as read
 * - task/message navigation uses one visible target highlight
...
@media (min-width:769px) and (pointer:fine){
  ...
  #activityFeedPanel .nfa-v15-mark-read{min-height:28px;padding:0 9px;border:1px solid #c7d2fe;border-radius:7px;background:#eef2ff;color:#4338ca;font-size:10px;font-weight:750;cursor:pointer;white-space:nowrap}
...
function plainNote(note){let value=String(note||'').trim();if(value.startsWith('[NFA_NOTE]'))value=value.slice('[NFA_NOTE]'.length);if(!/[<>]/.test(value))return value;const d=document.createElement('div');d.innerHTML=value;return String(d.innerText||d.textContent||'').trim()}
function parseTaskFile(raw){const text=String(raw||'').replace(/\u00a0/g,' ').trim();const marker=text.indexOf('|tasks/');if(marker<0)return null;const before=text.slice(0,marker).trim(),rest=text.slice(marker+1);const sep=rest.indexOf('|');const path=(sep>=0?rest.slice(0,sep):rest).trim();const note=plainNote(sep>=0?rest.slice(sep+1):'');if(!path.startsWith('tasks/'))return null;const fileAt=before.toLowerCase().lastIndexOf('file uploaded');let prefix='',name='';if(fileAt>=0){prefix=before.slice(0,fileAt).replace(/[—–\-:\s]+$/,'').trim();name=before.slice(fileAt+'file uploaded'.length).replace(/^[\s—–:\-]+/,'').trim()}else{name=before.split(/[—–]/).pop().trim()}if(!name)name=path.split('/').pop()||'Attachment';return{prefix,name,path,note}}
...
function ensureWebMarkAllRead(){const actions=$('#activityFeedPanel .nfa-af-actions');if(!actions||$('.nfa-v15-mark-read',actions))return false;const b=document.createElement('button');b.type='button';b.className='nfa-v15-mark-read';b.textContent='Mark all read';b.addEventListener('click',async()=>{b.disabled=true;try{const {error}=await sb().from('notifications').update({is_read:true}).eq('user_id',uid()).eq('is_read',false);if(error)throw error;window._setBellBadge?.(0);await window._loadActivityFeed?.();toast('Activity marked as read.')}catch(e){toast(e?.message||'Could not mark activity as read.',true)}finally{b.disabled=false}});const clear=$('.nfa-af-clear-all',actions);clear?actions.insertBefore(b,clear):actions.prepend(b);return true}
function installActivityOwner(){const owner=window.openActivityFeed;if(typeof owner!=='function')return false;if(owner.__nfaV15Read)return true;const wrapped=async function(){const result=await owner.apply(this,arguments);ensureWebMarkAllRead();return result};wrapped.__nfaV15Read=true;wrapped.__nfaOriginal=owner;window.openActivityFeed=wrapped;return true}
...
function install(){if(!desktop())return;installCss();installBadge();ensureTopModeSwitch();installMessageObserver();wrapRenderOwner('renderMessages');wrapRenderOwner('nfaRefreshTaskMessages');installReminderOwner();installActivityOwner();ensureWebMarkAllRead();installTaskOriginalOwner();installFrames++;if(installFrames<360&&(!$('#nfaConversationModeV8')||typeof window.sendTaskReminder!=='function'||typeof window.openTaskOriginalMessage!=='function'||typeof window.openActivityFeed!=='function'))requestAnimationFrame(install)}
requestAnimationFrame(install);
window.addEventListener('focus',()=>{ensureTopModeSwitch();decorateFiles($('#messagesContainer')||document);ensureWebMarkAllRead()});
