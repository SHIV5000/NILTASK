...
const mobileUnreadTest = read('scripts/test-mobile-unread-handoff.mjs');
...
contains(unreadService, 'function activateMobileHandoff()', 'Unread service exposes explicit mobile handoff');
contains(unreadService, 'mobileComputeAdapter.__nfaUnreadServiceMobileAdapter = true;', 'Mobile room compatibility call delegates to UnreadService');
contains(unreadService, 'mobileAttentionAdapter.__nfaUnreadServiceMobileAdapter = true;', 'Mobile attention compatibility call delegates to UnreadService');
contains(unreadService, 'QUERY.computeRoomUnread(sb,', 'Shared refresh calls the captured room query exactly once');
contains(unreadService, 'QUERY.unreadCount(sb, current.userId)', 'Shared refresh calls the captured attention query exactly once');
contains(unreadService, 'if (STATE.mobileHandoff) STATE.mobileCoalescedCalls += 1;', 'Paired mobile calls coalesce without scheduling another refresh');
contains(unreadService, 'mobileUsesExistingQueries: false', 'Unread snapshot rejects independent mobile query ownership');
contains(unreadService, 'mobileUsesSharedRefresh: STATE.mobileHandoff', 'Unread snapshot reports shared mobile refresh ownership');
contains(unreadService, 'mobileOwnsPolling: false', 'Unread service declares no mobile poll ownership');
contains(unreadService, 'mobileOwnsRendering: false', 'Unread service declares no mobile renderer ownership');
contains(unreadService, 'mobileOwnsAppBadge: false', 'Unread service declares no mobile app-badge ownership');
contains(unreadService, "window.addEventListener('niltask:unread-updated', consumeMobileUpdate);", 'Mobile handoff consumes the canonical unread event');
contains(unreadService, 'if (!isMobileRuntime()) {', 'Unread publication remains DOM-passive on mobile');
...
contains(mobileDiagnostics, 'sharedUnreadUsesOneRefresh', 'Mobile diagnostic verifies shared unread refresh ownership');
contains(mobileDiagnostics, 'sharedUnreadBypassesIndependentQueries', 'Mobile diagnostic rejects independent helper ownership');
contains(mobileDiagnostics, 'sharedUnreadHasNoOwnPoll', 'Mobile diagnostic verifies no second unread poll');
contains(mobileDiagnostics, 'sharedUnreadHasNoOwnRenderer', 'Mobile diagnostic verifies one mobile renderer');
contains(mobileDiagnostics, 'sharedUnreadHasNoOwnAppBadge', 'Mobile diagnostic verifies one mobile app-badge writer');
...
contains(mobileUnreadTest, 'paired mobile calls must perform one room query', 'Behavioral test proves one room query per paired mobile refresh');
contains(mobileUnreadTest, 'mobile coalescing must not schedule a second attention query', 'Behavioral test rejects a hidden second mobile refresh');
contains(mobileUnreadTest, 'shared service must not write the mobile app badge', 'Behavioral test preserves the single mobile badge writer');
contains(mobileUnreadTest, 'windowTarget.unreadCounts.staff, 5', 'Behavioral test preserves the live mobile unread floor');
