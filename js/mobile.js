window.NILTASK_MOBILE_VERSION = 'v208';
...
async function _activity(p) {
    ...
    const timelineCard = (it) => {
        ...
        return `<div class="af-card ${it.cls}${seen?' seen':''}" data-v208-icon="${it.emoji||'⚡'}" ${openData(it)}>
    ...
    const html = `<div class="mScr-inner ${mode==='attention'?'nf-mode':'af-mode'}">
      <div class="m-hdr m-hdr-plain" style="display:flex;align-items:center;justify-content:space-between;">
        <div class="m-htitle">${mode==='attention'?'🔔 Notifications':'🗞️ Activity'} <span class="af-version">v208</span> ${(mode==='attention'&&unread)?`<span style="background:#2563eb;color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:30px;margin-left:6px;vertical-align:middle;">${unread} new</span>`:''}</div>
