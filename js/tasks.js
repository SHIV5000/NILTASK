...
window.taskAction = async function(taskId, assigneeId, action, requireProof = false) {
    let actionText = '', newStatus = '', comment = '';
...
window.loadTasksForPanel = async function() {
    ...
    if (ids.length === 0) {
        document.getElementById('tasksPanel').innerHTML = '<p class="text-sm opacity-50 text-center mt-6"><i class="fa-solid fa-inbox text-3xl mb-2 block"></i> No tasks active.</p>';
        return;
    }
    ...
    if (assignees) assignees.forEach(a => {
        ...
        assigneesMap[a.task_id].push({ assignee_id: a.assignee_id, status: a.status, profiles: a.profiles });
    });

    const trailsMap = {};
    if (trails) trails.forEach(t => {
        if (!trailsMap[t.task_id]) trailsMap[t.task_id] = [];
        trailsMap[t.task_id].push(t);
    });

    const filterVal = document.getElementById('taskFilter')?.value || 'all';
    const sortVal = document.getElementById('taskSort')?.value || 'deadline_asc';
    ...
    let filteredTasks = tasks.filter(t =>
        t.assigned_by === window.currentUser.id ||
        (assigneesMap[t.id] && assigneesMap[t.id].some(a => a.assignee_id === window.currentUser.id))
    );
    ...
    filteredTasks.sort((a, b) => {
        ...
        if (sortVal === 'deadline_asc') {
            if (noDeadline(a) && noDeadline(b)) return 0;
            if (noDeadline(a)) return 1;
            if (noDeadline(b)) return -1;
    ...
    if (!filteredTasks.length) {
        container.innerHTML = '<p class="text-sm opacity-50 text-center mt-6"><i class="fa-solid fa-inbox text-3xl mb-2 block"></i> No tasks active.</p>';
        return;
    }
    ...
    container.innerHTML = filteredTasks.map(task => {
        ...
        let badgeClass = 'badge-pending', badgeText = '⏳ Needs Ack', borderClass = '#ea580c';
        ...
        if (task.deadline) {
            let d = new Date(task.deadline); d.setHours(23, 59, 59, 999);
        ...
        const assigneesNames = asgList.map(a => {
            let suffix = '', colorCls = 'text-gray-800';
